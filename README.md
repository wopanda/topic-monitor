...
历史上还出现过两个旧脚本：
- `scripts/legacy/topic-monitor.sh`：早期“投递 AI 任务 / 后台处理”的原型，不是现在 4.1 的主链。
- `scripts/legacy/topic-monitor-ai.sh`：早期“生成提示词、再交给 AI 执行”的原型，也不是现在 4.1 的主链。

所以你现在只需要围绕 `topic-monitor-run.sh` 理解、配置和运行；旧脚本仅保留作演进参考，不建议读者直接使用。
